...
pub fn upsert_candles(
    ...
    for r in rows {
        stmt.execute(params![
            ...
            r.ema9, r.ema20, r.ema50, r.ema100, r.ema200,
...
pub fn read_candles(
    ...
    let rows = stmt.query_map(params![symbol, timeframe, limit as i64], |row| {
        Ok(CandleV3 {
            ...
            rsi9:           row.get(22)?,
            rsi14:          row.get(23)?,
            rsi_trend:      row.get(24)?,
            stoch_rsi_k:    row.get(25)?,
            stoch_rsi_d:    row.get(26)?,
            cci:            row.get(27)?,
            pivot_point:    row.get(28)?,
            r1:             row.get(29)?,
            r2:             row.get(30)?,
            r3:             row.get(31)?,
            s1:             row.get(32)?,
            ...
            atr14:          row.get(39)?,
            bb_upper:       row.get(40)?,
            bb_middle:      row.get(41)?,
            bb_lower:       row.get(42)?,
            keltner_upper:  row.get(43)?,
            keltner_middle: row.get(44)?,
            keltner_lower:  row.get(45)?,
            hist_vol:              row.get(46)?,
            price_above_cloud: row.get::<_, i32>(47)? != 0,
            price_above_kijun: row.get::<_, i32>(48)? != 0,
            inside_bar:        row.get::<_, i32>(49)? != 0,
            rsi_divergence:    row.get::<_, i32>(50)? != 0,
        })
    }).map_err(|e| e.to_string())?;
