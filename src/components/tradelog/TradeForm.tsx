...
function JournalTab({ values, set }: TabProps) {
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Broker sync notes — auto-generated by TradeLocker/OANDA sync (partial-close
          legs, P&L estimate debug), kept separate from Technical notes so it never
          overwrites or clutters the user's own trade commentary there. */}
      <FormField label="Broker sync notes">
        <Textarea
          value={values.syncNotes}
          onChange={(v) => set("syncNotes", v)}
          placeholder="Auto-filled by TradeLocker/OANDA sync for trades synced from a broker…"
          rows={3}
        />
      </FormField>
    </div>
